...
let cart = JSON.parse(localStorage.getItem('phooldhara_cart')) || [];
let isCartCheckout = false;
let currentOrder = { product: '', price: 0, fragrance: '' };

// ===== PREMIUM SHOPPING CART FUNCTIONS =====
...
function addToCart(productName, price, fragrance, qty) {
  const existingIdx = cart.findIndex(item => item.productName === productName && item.fragrance === fragrance);
...
function updateCartQty(idx, delta) {
  ...
  if (cart[idx].quantity > 50) cart[idx].quantity = 50;
  saveCart();
  renderCart();
}

function saveCart() {
  localStorage.setItem('phooldhara_cart', JSON.stringify(cart));
}
...
function renderCart() {
  ...
  cartDrawerItems.innerHTML = cart.map((item, idx) => `
    <div class="cart-item">
      <div class="cart-item-details">
        <div class="cart-item-name">${item.productName}</div>
        <div class="cart-item-fragrance">🌸 Fragrance: ${item.fragrance || 'N/A'}</div>
...
function openCartCheckout() {
  ...
  document.getElementById('modal-product-name').textContent = "Multi-product Order 🛒";
  document.getElementById('modal-fragrance').innerHTML = cart.map(item => `• ${item.productName} (${item.fragrance} × ${item.quantity})`).join('<br>');
  document.getElementById('modal-price').textContent = '₹' + subtotal.toLocaleString('en-IN');
  
  const btnAmt = document.getElementById('btn-amount');
  if (btnAmt) btnAmt.textContent = subtotal;
...
function openPayModal(productName, price, btn) {
  ...
  var fragrance = sel ? sel.options[sel.selectedIndex].text : '';
  
  isCartCheckout = false;
  
  // Add to cart directly and open drawer!
  addToCart(productName, price, fragrance, qty);
}
...
async function placeCODOrder() {
  ...
  if (isCartCheckout) {
    totalPrice = calculateCartSubtotal();
    itemsPayload = cart.map(item => ({
    ...
    whatsappMsg = 'Namaste! 🙏 I would like to place a COD order:\n\n' +
      cart.map(item => `• *Product*: ${item.productName}\n  *Fragrance*: ${item.fragrance}\n  *Quantity*: ${item.quantity}\n  *Price*: ₹${item.price * item.quantity}\n`).join('\n') +
    ...
    totalPrice = currentOrder.price * (currentOrder.quantity || 1);
    itemsPayload = [{
  ...
  try {
    await fetch(`${BACKEND_URL}/api/cod-order`, {
      ...
      body: JSON.stringify({
        customerName:  name,
        customerPhone: phone,
        productName:   isCartCheckout ? "Multi-product Order" : currentOrder.product,
        fragrance:     isCartCheckout ? "" : (currentOrder.fragrance || ''),
        quantity:      isCartCheckout ? 1 : (currentOrder.quantity || 1),
        amount:        totalPrice,
        address:       address,
        items:         itemsPayload
      })
  ...
  closePayModal();
  window.open('https://wa.me/916375507029?text=' + encodeURIComponent(whatsappMsg), '_blank');
}
...
async function initiateRazorpay() {
  ...
  try {
    ...
    if (!data.success) {
      ...
      btn.disabled  = false;
      btn.innerHTML = `💳 Pay ₹<span id="btn-amount">${totalPrice}</span> Securely`;
      return;
    }
    ...
    const options = {
      ...
      name:        'PhoolDhara 🌸',
      description: isCartCheckout ? "Multi-product Order" : currentOrder.product + (currentOrder.fragrance ? ' — ' + currentOrder.fragrance : ''),
