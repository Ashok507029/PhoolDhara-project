...
document.querySelectorAll('input[name="payMethod"]').forEach(function(radio) {
  radio.addEventListener('change', function() {
    ...
    if (this.value === 'cod') {
      ...
      payBtn.innerHTML = '💳 Pay ₹' + document.getElementById('btn-amount').textContent + ' Securely';
...
function openPayModal(productName, price, btn) {
  ...
  document.getElementById('btn-amount').textContent         = price * qty;
...
async function initiateRazorpay() {
  ...
  try {
    ...
    if (!data.success) {
      ...
      btn.disabled  = false;
      btn.innerHTML = `💳 Pay ₹${currentOrder.price} Securely`;
      return;
    }
