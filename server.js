...
const reviewSchema = new mongoose.Schema({
  ...
  approved: { type: Boolean, default: false }, // Moderation: only approved reviews go live
}, { timestamps: true });
...
app.post('/api/create-order', async (req, res) => {
  try {
    const { amount, productName, fragrance, quantity, customerName, customerPhone, address, items } = req.body;
...
app.post('/api/cod-order', async (req, res) => {
  try {
    const { customerName, customerPhone, productName, fragrance, quantity, amount, address, items } = req.body;
...
// ==============================
// ROUTE: Get All Reviews (Approved Only)
// ==============================
...
// ==============================
// ROUTE: Get All Reviews for Admin (Both Approved & Pending)
// ==============================
...
// ==============================
// ROUTE: Approve/Hide a Review (Admin)
// ==============================
app.put('/api/admin/reviews/:id/approve', async (req, res) => {
  try {
    ...
    console.log(`[REVIEW MODERATION] ${updatedReview.name}'s review approved status set to ${updatedReview.approved}`);
    res.json({ success: true, review: updatedReview });
  } catch (error) {
    console.error('Review Approve Error:', error);
    res.status(500).json({ success: false, message: 'Review update status failed.' });
  }
});
